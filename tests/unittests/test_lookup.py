...
from tests.utils import (
    send_test_set_cell_request,
    get_cell_from_database,
    tear_down_database,
)
...
class TestLookup(unittest.TestCase):
    ...
    def test_set_cell_lookup_valid(self):
        ...
        response, status_code = send_test_set_cell_request(
            self.sheet_id, "col2", 3, "LOOKUP(col2,1)"
        )
        ...
        response, status_code = send_test_set_cell_request(
            self.sheet_id, "col2", 4, "LOOKUP(col2,3)"
        )
        self.assertEqual(status_code, 201)
    ...
    def test_set_cell_lookup_wrong_pattern(self):
        ...
        response, status_code = send_test_set_cell_request(
            self.sheet_id, "col2", 3, "LOOKUP(col2)"
        )
        self.assertIn(
            "string starts with 'LOOKUP' but does not match the pattern", response
        )
        self.assertEqual(status_code, 400)

        response, status_code = send_test_set_cell_request(
            self.sheet_id, "col2", 3, "LOOKUP(col2,1,2)"
        )
        # will take the second arg as string if "1,2"
        self.assertIn(
            "Given row number in LOOKUP(col,row_number) is not an int.", response
        )
        self.assertEqual(status_code, 400)
    ...
    def test_set_cell_lookup_with_cyclict_dependent(self):
        ...
        response, status_code = send_test_set_cell_request(
            self.sheet_id, "col2", 1, "LOOKUP(col2,4)"
        )
        self.assertEqual(status_code, 400)
